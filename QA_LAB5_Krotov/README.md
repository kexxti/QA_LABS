...
![post_secrets_auth](./misc/media/post_secrets_auth_token.png)
...
![post_secrets_response](./misc/media/post_secrets_response.png)
...
В ходе лабораторной работы были изучены основы тестирования REST API с использованием Postman.

В качестве тестируемого сервиса был выбран учебный Secrets API, поддерживающий несколько способов аутентификации: Basic Authentication, API Key и Bearer Token Authentication.

Во время работы были созданы и протестированы запросы для:

- регистрации пользователя,
- получения токена авторизации,
- получения API ключа,
- получения списка секретов,
- создания новых секретов,
- удаления секретов,
- проверки корректности работы API после выполнения операций.

Также были изучены возможности Postman по автоматизации тестирования.

В ходе выполнения работы стало понятно, что тестирование API — это не только отправка отдельных HTTP-запросов, но и построение последовательных сценариев взаимодействия с сервером, учитывающих состояние системы и передачу данных между запросами.
